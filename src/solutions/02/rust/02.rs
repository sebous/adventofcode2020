...
fn validate_p1(pass_info: &PasswordInfo) -> bool {
    let letter_count = pass_info
        .password
        .chars()
        .filter(|&ch| ch == pass_info.letter)
        .count() as u32;
...
fn validate_p2(pass_info: &PasswordInfo) -> bool {
    let first_char = pass_info
        .password
        .chars()
        .nth((pass_info.min - 1) as usize)
        .unwrap();
    let second_char = pass_info
        .password
        .chars()
        .nth((pass_info.max - 1) as usize)
        .unwrap();
...
pub fn run() {
    let input = load_input("src/solutions/02/data.txt")
        .lines()
        .map(|l| {
            let splitted_line = l.split(" ").collect::<Vec<&str>>();

            PasswordInfo {
                min: splitted_line[0].split("-").collect::<Vec<&str>>()[0]
                    .parse()
                    .unwrap(),
                max: splitted_line[0].split("-").collect::<Vec<&str>>()[1]
                    .parse()
                    .unwrap(),
